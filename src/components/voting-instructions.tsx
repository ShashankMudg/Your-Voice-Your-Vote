      ...
      <CardContent>
        <form ref={formRef} action={formAction} className="space-y-4">
          <div className="space-y-2">
            ...
            {state.validationErrors?.age && <p className="text-sm font-medium text-destructive mt-1">{state.validationErrors.age[0]}</p>}
          </div>
          <div className="space-y-2">
            <Label htmlFor="location">Your Location (City/State)</Label>
            <Input id="location" name="location" placeholder="e.g., Delhi" required />
            {state.validationErrors?.location && <p className="text-sm font-medium text-destructive mt-1">{state.validationErrors.location[0]}</p>}
          </div>
          <SubmitButton />
        </form>
